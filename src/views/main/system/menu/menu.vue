<template>
  <div class="menu">
    <page-content ref="pageContentRef" :contentTableConfig="contentTableConfig" isDgut="true" pageName="permission"
      urlPre="/auth" :requestInfo="requestInfo" :listOtherParams="{ paginated: false, treed: true }"
      :allPermissionBtn='allPermissionBtn' @newBtnClick="handleNewData" @editBtnClick="handleEditData">
      <!-- <template #level="scope">{{
          scope.row.level == 1 || scope.row.level == 2 ? `${scope.row.level}级菜单` : '权限'
      }}</template> -->
    </page-content>
    <page-model ref="pageModalRef" pageName="permission" :modelConfig="SwitchModelConfig" :defaultInfo="defaultInfo"
      :requestInfo="requestInfo" @confirmClick='handleConfirm' :listOtherParams="{ paginated: false, treed: true }">
    </page-model>
  </div>
</template>
...
import { getAuthMenuList, addMenuRequest, addPageRequest, addPermissionRequest,updateMenuAllReq } from '@/serve/DgutRequest/menu/index'
import { usePageModal } from '@/hooks/use-page-modal'
import { menuViewConfig, pageViewConfig, permissionViewConfig } from './config/modal/Viewconfig'
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleNewData = (type: any, item: any) => {
      ...
      switch (type) {
        case 'add_menu': SwitchModelConfig.value = menuViewConfig; break;
        case 'add_page': SwitchModelConfig.value = pageViewConfig; break;
        case 'add_permission': SwitchModelConfig.value = permissionViewConfig; break;
      }

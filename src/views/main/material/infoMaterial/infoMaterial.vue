<template>
  <div class="user">
    <page-search :searchFormConfig="searchFormRef" @resetBtnClick="handleResetClick" @queryBtnClick="handleQueryClick"
      @changeQueryInfo="handleChangeQueryInfo" ref="pageSearchRef" />
    <page-content ref="pageContentRef" :contentTableConfig="contentTableConfig" pageName="material"
      ...
      :isPrint="true" :requestInfo="requestInfo" :allPermissionBtn="allPermissionBtn" :queryCb="queryCb">
      <template #image="scope">
        <el-image style="width: 60px; height: 60px" :src="scope.row.photo ? BASE_IMG_URL + scope.row.photo : ''"
          :z-index="99999" :preview-src-list="[scope.row.photo ? BASE_IMG_URL + scope.row.photo : '']"></el-image>
      </template>
...
export default defineComponent({
  ...
  setup() {
    const isUpdate = usePermission('material', 'update')
    ...
    const editCallFn = (item: any, defaultInfo: any) => {
      ...
      defaultInfo.value = {
        ...
        photo: defaultInfo.value.photo
          ? BASE_IMG_URL + defaultInfo.value.photo
          : ''
      }
      //动态请求物资详情
    }
    const addCallFn = (item: any, defaultInfo?: any) => {
    }
    const queryCb = (formData: any) => {
      ...
      if (queryParams['gmtWarehoused']) {
        queryParams['startWarehoused'] = formatUtcString(
          queryParams['gmtWarehoused'][0]
        )
        queryParams['endWarehoused'] = formatUtcString(
          queryParams['gmtWarehoused'][1]
        )
        delete queryParams['gmtWarehoused']
      }
      if (queryParams['gmtBought']) {
        queryParams['startBought'] = formatUtcString(
          queryParams['gmtBought'][0]
        )
    ...
    const [pageModalRef, defaultInfo, handleNewData, handleEditData] =
      usePageModal(addCallFn, editCallFn)
